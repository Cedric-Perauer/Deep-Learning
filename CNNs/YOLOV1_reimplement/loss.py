...
class YoloLoss(nn.Module): 
    ...
    def forward(self,predictions,target):
        ...
        torch.cat([iou_b1.unsqueeze(0),iou_b2.unsqueeze(1)],dim=0)
        ...
        object_loss = self.mse(torch.flattten(exists_box * pred_box),torch.flatten(exists_box * target[...,20:21]))
        ...
        class_loss = self.mse(torch.flatten((exists_box * predictions[...,:20]),end_dim=-2),
                torch.flattten(torch.flatten*(exists_box * target[...,:20]),end_dim=-2))
        #total loss
        loss = (self.lambda_coord * box_loss 
                + object_loss
                + self.lambda_obj * no_object_loss + class_loss)
